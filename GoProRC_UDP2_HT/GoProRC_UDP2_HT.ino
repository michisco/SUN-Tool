...
uint8_t Cam1Mac[6] = {0xF4, 0xDD, 0x9E, 0xDB, 0xA7, 0xAF};
uint8_t Cam2Mac[6] = {0xF4, 0xDD, 0x9E, 0xE1, 0x27, 0x87};
uint8_t Cam3Mac[6] = {0x04, 0x41, 0x69, 0x37, 0x28, 0xA1};
uint8_t Cam4Mac[6] = {0xF4, 0xDD, 0x9E, 0xE1, 0x25, 0x11};
...
uint8_t pcMac[6] = {0x54, 0x8C, 0xA0, 0x1D, 0x32, 0x3F};
uint8_t *ignoreMacArray[4] = {emptyMac, emptyMac, emptyMac, emptyMac}; //54:8C:A0:1D:32:3F
//
...
uint8_t outputPin0 = D5;
uint8_t outputPin1 = D6;
uint8_t outputPin2 = D7;
uint8_t outputPin3 = D8;
...
void setup() {
  ...
  analogWrite(outputPin0, dutyCycle);  /* set initial 50% duty cycle */
  analogWrite(outputPin1, dutyCycle);  /* set initial 50% duty cycle */
  analogWrite(outputPin2, dutyCycle);  /* set initial 50% duty cycle */
  analogWrite(outputPin3, dutyCycle);  /* set initial 50% duty cycle */
}
...
void loop() {
  ...
  analogWrite(outputPin0, dutyCycle);
  analogWrite(outputPin1, dutyCycle);
  analogWrite(outputPin2, dutyCycle);
  analogWrite(outputPin3, dutyCycle);
}
...
void receiveFromSerial() {
  ...
  if (ReadSerialMonitorString(sString) > 0) {
    ...
    }  else if (strstr_P(sString, PSTR("light")) != NULL) {
      
      char *res = strchr(sString, ' ');
      dutyCycle = atoi(res + 1);
